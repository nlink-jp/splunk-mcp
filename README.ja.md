...
`invalid_arguments` で失敗し、その名前を挙げます（`unknown field "max_rowz"`）。
とくに `max_rows` で影響が大きく、綴り間違いは設定の既定値へフォールバックした
まま「指定した上限が効いた」ように見えていました —— このサーバーが存在する理由
である「件数を明示する」ことの正反対です。型が違う引数も同様に拒否され、引数の
デコードより前に Splunk へは一切アクセスしないので、拒否された呼び出しは検索
ジョブを開始しません。引数をまったく渡さない呼び出しは従来どおり「引数なし」
として扱われます。
...
（確定件数・`max_rows` によるキャップとその計上・非同期フロー）を検証します。詳細は
